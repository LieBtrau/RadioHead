...
RH_SwSerial::RH_SwSerial(SoftwareSerial& serial)
    :
    _serial(serial),
...
SoftwareSerial& RH_SwSerial::serial()
{
    return _serial;
}

bool RH_SwSerial::init()
{
...
void RH_SwSerial::waitAvailable()
{
...
bool RH_SwSerial::waitAvailableTimeout(uint16_t timeout)
{
#if (RH_PLATFORM == RH_PLATFORM_UNIX)
    // Unix version driver in RHutil/HardwareSerial knows how to wait without polling
...
void  RH_SwSerial::handleRx(uint8_t ch)
{
...
void RH_SwSerial::appendRxBuf(uint8_t ch)
{
...
void RH_SwSerial::validateRxBuf()
{
...
bool RH_SwSerial::recv(uint8_t* buf, uint8_t* len)
{
    if (!available())
	return false;
...
bool RH_SwSerial::send(const uint8_t* data, uint8_t len)
{
    if (!waitCAD()) 
...
void  RH_SwSerial::txData(uint8_t ch)
{
...
uint8_t RH_SwSerial::maxMessageLength()
{
